...
* Navigate RStudio
* Pledge to always use R Projects
* Understand R's types and structures
* Manipulate data frames
* Conduct basic EDA (with plots!)
...
* DataSF uses R to manage and publish the [San Francisco Population and Demographic Census Data.](https://data.sfgov.org/Economy-and-Community/San-Francisco-Population-and-Demographic-Census-Da/4qbq-hvtt/about_data)

* DPH uses R to clean, geocode, and analyze survey and patient data. 

* DPW uses R make both static and interactive maps.

* OEWD uses R to read data from Salesforce, Airtable, and other third-party systems and APIs, and build robust data pipelines.  
...
```
WARNING: Rtools is required to build R packages but is not currently installed. Please download and install the appropriate version before proceeding:
```

Rtools includes a handful of 'compilers' for installing packages 'from source' on Windows. In brief, you might need to install a package 'from source' because the usual 'binaries' for a required version are not available. If you're on Windows, it's best to [install Rtools.](https://cran.r-project.org/bin/windows/Rtools/)

[See here for a helpful discussion on installing R packages 'from source'.](https://forum.posit.co/t/having-trouble-installing-and-loading-tidyverse-readr-no-hms-package/11268/2)
...
A **warning** warns that something less-than-ideal might have occurred upon successful execution.
...
R has three numeric types: numeric (or 'double' floating point), integer, and complex.
...
**`iris`** - measurements of iris sepal and petal lengths/widths.
...
**`mtcars`** - aspects of automobile design and performance for 32 vehicles.

```{r}
str(mtcars)
```

See also: `cars`, `airquality`, `USArrests`, `InsectSprays`, etc.
...
1. The first argument is always the data frame.
2. The next arguments say what to do with the data frame.
3. They return a data frame.
4. They don't 'modify in place.' 
...
[ggplot2.R](https://github.com/SFOEWD/data-academy-intro-to-r/blob/main/R/ggplot2.R)
